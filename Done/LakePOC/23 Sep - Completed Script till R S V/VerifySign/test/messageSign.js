...
//const Validation = require('Validation.sol');
...
async function startApp() {
  ...
  try {
    ...
    const messageP = "Hello world!";
    const message = web3.utils.toHex(messageP); // "0x48656c6c6f20776f726c6421"
    ...
    let instance = new web3.eth.Contract(abi, '0x60b83B39357f28390aA817C3771fC85c0907Bbd3');

    let fixed_msg = `\x19Ethereum Signed Message:\n${messageP.length}${messageP}`;
    let fixed_msg_sha = await web3.utils.sha3(fixed_msg);

    let publicSignature = instance.recoverAddr.call(
      fixed_msg_sha,
      v_decimal,
      r,
      s
